...
public class Crawl implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) {
        ...
        if (args.length > 0) {
            ...
            if (target == null) {
                sender.sendMessage(Other.Tools.chat(b + "[" + n + "CRAWL" + b + "]" + t + " Player " + h + args[0] + t + " is not online!"));
                return false;
            }
            who = "Player";
        } else {
            if (sender instanceof Player) {
                ...
                sender.sendMessage(Other.Tools.chat(b + "[" + n + "CRAWL" + b + "]" + t + " Please use " + h + "/crawl <player>"));
                return false;
            }
        }
        ...
        if ((!Objects.equals(target.getMetadata("crawl").toString(), "[]") && !target.getMetadata("crawl").getFirst().asBoolean()) || Objects.equals(target.getMetadata("crawl").toString(), "[]")) {
            if (target.isOnGround()) {
                ...
                sender.sendMessage(Other.Tools.chat(b + "[" + n + "CRAWL" + b + "]" + t + " " + who + " must stand on a block!"));
            }
        } else {
            Other.bDisable(target, false);
        }
